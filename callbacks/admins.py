...
async def coins_handler(
    call: CallbackQuery, bot: Bot, state: FSMContext, callback_data: AdminCallback
):
    ...
    if callback_data.action == "get_all_users":
        ...
        await call.message.edit_text(
            f"📝 Введите новую цену {callback_data.action}\n\nТекущая цена: {coin_info['cost']}",
            reply_markup=admin_currency_buttons,
        )
...
async def amount_handler(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        await message.answer(
            "❌ <b>Пожалуйста, введите корректное число</b>", reply_markup=cancel_button
        )
        return
