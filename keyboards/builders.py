    ...
    if not amount:
        ...
        for option in open_options:
            if box_balance >= option:
                builder.button(
                    text=f"Открыть {option} BOX",
                    callback_data=BoxCallback(type=CoinActions.OPEN, amount=option).pack(),
                )
                button_count += 1

        builder.button(
            text="➕ Купить", callback_data=BoxCallback(type=CoinActions.BUY, amount=None).pack()
        )
        ...
        builder.button(
            text=f"Открыть ещё {amount} BOX",
            callback_data=BoxCallback(type=CoinActions.OPEN, amount=amount).pack(),
        )
        builder.adjust(1)
